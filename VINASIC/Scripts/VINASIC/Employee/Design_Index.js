...
VINASIC.Design = function () {
    ...
    function updateDetailStatus(detailId, status, employeeUpdateId) {
        $.ajax({
            url: "/Order/EmployeeUpdateDetailStatus?detailId=" + detailId + "&status=" + status + "&employeeId=" + employeeUpdateId + "&updateType=" + 1,
            type: 'post',
        ...
        $("#" + global.Element.JtableDesign).jtable({
            ...
            fields: {
                ...
                strJob: {
                    ...
                    width: "10%",
                    display: function (data) {
                        if (data.record.DesignDescription!='') {
                            var text = $('<a style="color:red;" href="javascript:void(0)" class="clickable"  data-target="#popup_Design" title="">' + "Chi Tiết" + '</a>');
                        }
                        else {
                        ...
                        text.click(function () {
                            $("#view-content").empty();
                            $("#view-content").append(data.record.DesignDescription);
