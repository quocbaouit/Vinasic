...
namespace VINASIC.Controllers
{
    public class OrderDetailStatusController : BaseController
    {
        ...
        public OrderDetailStatusController(IBllOrderDetailStatus bllOrderDetailStatus)
        {
        ...
        public JsonResult GetOrderDetailStatuss(string keyword, int jtStartIndex, int jtPageSize, string jtSorting)
        {
            try
            {
                ...
                var listOrderDetailStatus = _bllOrderDetailStatus.GetList(keyword, jtStartIndex, jtPageSize, jtSorting);
        ...
        public JsonResult SaveOrderDetailStatus(ModelOrderDetailStatus modelOrderDetailStatus)
        {
            try
            {
                if (IsAuthenticate)
                {
                    ...
                    if (modelOrderDetailStatus.Id == 0)
                    {
                        ...
                        responseResult = _bllOrderDetailStatus.Create(modelOrderDetailStatus);
                    }
                    else
                    {
                        modelOrderDetailStatus.UpdatedUser = UserContext.UserID;
                        responseResult = _bllOrderDetailStatus.Update(modelOrderDetailStatus);                    
                    }
        ...
        public JsonResult DeleteOrderDetailStatus(int id)
        {
            try
            {
                if (IsAuthenticate)
                {
                    var responseResult = _bllOrderDetailStatus.DeleteById(id, UserContext.UserID);
                    if (responseResult.IsSuccess)
                        JsonDataResult.Result = "OK";
                    else
                    {
